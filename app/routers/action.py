...
def import_all_data(usage_point_id):
    return Ajax(CONFIG, DB, usage_point_id).import_data()
...
def delete_all_data(usage_point_id):
    return Ajax(CONFIG, DB, usage_point_id).delete_all_data()
...
def reset_gateway(usage_point_id):
    return Ajax(CONFIG, DB, usage_point_id).reset_gateway()
...
def reset_data(usage_point_id, target, date):
    return Ajax(CONFIG, DB, usage_point_id).reset_data(target, date)
...
def blacklist_data(usage_point_id, target, date):
    return Ajax(CONFIG, DB, usage_point_id).blacklist(target, date)
...
def whitelist_data(usage_point_id, target, date):
    return Ajax(CONFIG, DB, usage_point_id).whitelist(target, date)
...
def fetch_data(usage_point_id, target, date):
    return Ajax(CONFIG, DB, usage_point_id).fetch(target, date)
