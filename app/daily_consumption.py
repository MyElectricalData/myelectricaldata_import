    ...
    for key, value in data.items():
        ...
        if base_price != 0:
            if isinstance(value, int):
                ...
                f.publish(client, f"{pdl}/consumption_price/current_year/{key}", roundValue)
                if key != "dateBegin" and key != "dateEnded":
    ...
    while current_year <= main.years:
        if main.years >= current_year:
            ...
            if "error_code" in data:
                f.publish(client, f"{pdl}/consumption/year-{current_year}/error", str(1))
                for key, value in data.items():
                    f.publish(client, f"{pdl}/consumption/year-{current_year}/errorMsg/{key}", str(value))
            else:
                f.publish(client, f"{pdl}/consumption/year-{current_year}/error", str(0))
                for key, value in data.items():
                    ...
                    if base_price != 0:
                        if isinstance(value, int):
                            roundValue = round(int(value) / 1000 * base_price, 2)
                            f.publish(client, f"{pdl}/consumption_price/year-{current_year}/{key}", roundValue)
